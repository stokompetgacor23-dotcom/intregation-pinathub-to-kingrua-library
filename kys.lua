...
if not ModernV2 then
    ...
    if loaderOk then
        ...
        warn("[QYZEL HUB] Vercel mirror failed, trying GitHub fallback:", loaderResult)
        ...
        if fallbackOk then"https://raw.githubusercontent.com/Kys-lol/KysHubNewUI/refs/heads/main/MainV2.lua"
            ModernV2 = fallbackResult
        else
            warn("[QYZEL HUB] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
local UIConfigPath = "QYZEL_HUB_UI_Theme.json"
...
if ModernV2 and ModernV2.CreateMenuIcon then
    MenuIcon = ModernV2:CreateMenuIcon({
        Image = "rbxassetid://104442518163067",
        Size = 48,
...
if ModernV2 then
    Window = ModernV2:Window({
        ...
        Config = {
            ConfigFolder = "QYZEL_HUB_VIOLENCE_DISTRICT",
            AutoSaveFile = "QYZEL_VD",
            AutoSave = false,
            AutoLoad = false,
            Overwrite = true,
            Format = "JSON",
    ...
    Window:SetAccount({
        ...
        Expires = "QYZEL HUB PREMIUM",
    })
    Window:CreateHomeTab({
        Name = "Dashboard",
        Icon = "lucide:layout-dashboard",
        Content = "QYZEL HUB • Violence District",
        DiscordInvite = "",
        ...
        UIChangelog = {
            {
                ...
                Date = "Latest",
                Description = "Added QYZEL Dark Theme",
            },
        }
    })
end
...
function VD_UpdateCrosshair()
    ...
    CrosshairGui.Name = "QYZEL_Crosshair"
...
function GetSafeChamsFolder()
    ...
    local f = pg:FindFirstChild("QYZEL_WorkspaceChams")
    if not f then
        f = Instance.new("Folder")
        f.Name = "QYZEL_WorkspaceChams"
        f.Parent = pg
    end
    VD_ChamsFolder = f
    return f
end
local ConfigFolderName = "QYZEL_HUB_VD"
local HttpService = game:GetService("HttpService")
...
print("QYZELHUB loaded")
pcall(function()
    VD_Notify("QYZELHUB loaded", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
function GetRole()
...
function KYS_ScanMap()
    ...
    print("[QYZEL ScanMap] Generators:", #newGens, "Gates:", #newGates, "Hooks:", #newHooks, "Windows:", #newWindows)
    local root           = Root
...
function KYS_TeleportToGenerator(index)
    if not KYS_Cache or not KYS_Cache.Generators or #KYS_Cache.Generators == 0 then print("[QYZEL HUB] Generator tidak ditemukan") return false end
    local sorted = {}
...
function KYS_TeleportToGate()
    if not KYS_Cache or not KYS_Cache.Gates or #KYS_Cache.Gates == 0 then print("[QYZEL HUB] Gate tidak ditemukan") return false end
    local closest, closestDist = nil, math.huge
...
function KYS_TeleportToHook()
    if not KYS_Cache or not KYS_Cache.ClosestHook then print("[QYZEL HUB] Hook tidak ditemukan") return false end
...
local function QYZEL_UnlockPremium()
    ...
    print("[QYZEL HUB] Premium features unlocked.")
end
